...
|----|---------|--------|-----------|------|------|
| EC-14 | 予約は2週間先から＋2週間以内は問い合わせフォームへ | 中 | DONE | Son | 2026-08-18完了（スキル enchakai-ops 定型2・実装subagent/検証Son実機）。①成果物=booking-window.ts(今日JST+14日カットオフの単一正本)・公開slots GETフィルタ＋予約POST 400拒否(多層防御)・予約ページにカレンダー下の2週間ルール告知＋#inquiryアンカー・問い合わせフォーム新設(POST /api/inquiries→data/inquiries.json保存+ホストへResend通知・顧客宛送信なし)・admin一覧API(GET /api/admin/inquiries)・en/ja両言語(key parity済) ②検証=build+tscゼロ・restart後 /en /ja booking 200・公開APIの最古枠=9/1(8/31以前非表示)・8/25指定POSTがカットオフ文言で400実測・問い合わせPOST→保存+admin一覧+ホスト通知メール実測・両ロケールで告知文とフォーム描画実測・既存予約2件無傷・テストデータ削除済 ③本番反映=済(restart+push済)。adminはカットオフ制限なし(手動対応用)。admin画面の問い合わせ一覧UIは未実装(メール通知が主経路・必要なら追補)。追補23:04(Keita FB即対応): 予約可能日(=2週間先以降)を選択したら告知バナー＋問い合わせフォームを非表示に(26ee4ef)。本番実機で選択前=表示/9月1日選択後=非表示を実測・push済 |
| EC-13 | PC用レスポンシブ対応（デスクトップレイアウト） | 中 | DONE | Son | 2026-08-18完了（スキル enchakai-ops 定型2・実装はsubagent委譲/検証はSon実機）。①成果物=md:/lg: の追加ブレークポイントのみで8ファイル改修（Header/Footer/Hero/EditorsNote/Ceremony/Host/ReserveBlock/PlanTiers）。真因=各セクションにハードコードされた mx-auto max-w-[640px]。ヘッダー全幅(ロゴ左/ナビ右)・ヒーロー21:9シネマ比率・Ceremony2カラム+プランカードsticky・Host画像テキスト横並び・フッター3カラム ②検証=build+tscゼロ・restart後 /en /ja /en/booking /ja/faq 全200・1440x900実画面3枚でデスクトップ描画確認・390x844でモバイル無変化を実画面確認・base クラス無変更(モバイル影響なし) ③本番反映=済（restart+push済）。booking頁は既存lg2カラムのため未変更・NeighborhoodsPreviewはデッドコードのため未変更 |
