...
| EC-3 | 送信ドメイン認証（顧客宛メール解放） | 中 | BLOCKED | Son | 2026-08-11起票。onboarding@resend.devは本人宛限定のため、顧客宛の受付/確定メールにはResendでドメイン認証が必要。型A: Resendダッシュボードでapollomansion.comのドメイン追加(Keita操作)→提示されるDNSレコード(SPF/DKIM 2-3件)をCloudflareへ追加(Keita操作 or CF APIトークン提供でSon実施)→EMAIL_FROM=bookings@apollomansion.com系へ切替・再検証はSon |
